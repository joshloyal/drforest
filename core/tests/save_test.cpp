...
TEST_CASE("Sliced Average Variance Estimation", "[save]") {
    auto [X, y] = drforest::testing::load_quadratic();
    arma::mat expected_directions =
        drforest::testing::load_quadratic_directions();

    arma::mat directions =
        drforest::fit_sliced_average_variance_estimation(X, y, 10);
...
TEST_CASE("Weighted Sliced Average Variance Estimation", "[save]") {
    auto [X, y] = drforest::testing::load_quadratic();
   ...
   arma::mat expected_directions =
    ...
    auto [eigvals, directions] =
        drforest::fit_sliced_average_variance_estimation(X, y_stats, sample_weight, 10);
...
TEST_CASE("Weighted Sliced Average Variance Estimation Boostrap", "[save]") {
    auto [X, y] = drforest::testing::load_quadratic();
    ...
    auto [eigvals, directions] =
        drforest::fit_sliced_average_variance_estimation(X, y_stats,
                                                         sample_weight, 10);
...
TEST_CASE("Weighted Sliced Average Variance Estimation Boostrap Subsample",
          "[subsample save]") {
    auto [X, y] = drforest::testing::load_quadratic();
    ...
    arma::mat X_sub = X.rows(0, 249);
    arma::vec y_sub = y.rows(0, 249);
    sample_weight = sample_weight.rows(0, 249);
    ...
    auto [y_stats, indices] = drforest::determine_target_stats(
        y_sub, sample_weight);
    auto [eigvals, directions] =
        drforest::fit_sliced_average_variance_estimation(X_sub, y_stats,
                                                         sample_weight, 10);

    // actually create the bootstrap
    X = X.rows(sample_indices);
    y = y.rows(sample_indices);
    arma::uvec index = arma::find(sample_indices == 249); // first 100 samples
