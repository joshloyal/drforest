...
g.savefig(os.path.join(OUT_DIR, 'Figure S.5.png'),
...
n_slices = np.sqrt(X.shape[0])
errors = np.zeros(len(min_sample_leaves))
...
importances = drforest.local_principal_direction(X_std, exclude_cols=[month_col], n_jobs=-1)
...
ax[1].legend(bbox_to_anchor=(0.5, 1.15), loc='upper center', ncol=4, facecolor='w', fontsize=16)
...
fig.savefig(os.path.join(OUT_DIR, 'Figure 4.png'), dpi=300,
            bbox_inches='tight')
...
fig.savefig(os.path.join(OUT_DIR, 'Figure S.6.png'),
           dpi=300, bbox_inches='tight')
